...
import { authenticateAdmin, normalizeUserRow, parsePositiveGb, parsePositiveInt, parseTimeoutSeconds } from "./utils.js";
...
export async function handleUpdateAdminUser(req, res, body) {
    ...
    const verifyTimeoutSeconds = parseTimeoutSeconds(abcVerifyTimeoutSeconds);
    const metricsTimeoutSeconds = parseTimeoutSeconds(abcMetricsTimeoutSeconds);

    if (!maxSingleBytes || !totalQuotaBytes || !maxBatchCount || !verifyTimeoutSeconds || !metricsTimeoutSeconds) {
        res.status(400).json({
            error: `Quota values and ABC timeouts must be positive numbers. Multi-file batch count must be an integer >= 1 (default ${DEFAULT_MAX_MULTI_FILE_BATCH_COUNT}).`,
        });
        return;
    }
    ...
    const update = await sql`
      ...
      set
        ...
        abc_verify_timeout_seconds = ${verifyTimeoutSeconds},
    ...
    await addActionLog({
        ...
        details: {
            ...
            abcVerifyTimeoutSeconds: Number(abcVerifyTimeoutSeconds),
            abcMetricsTimeoutSeconds: Number(abcMetricsTimeoutSeconds),
        },
    });
